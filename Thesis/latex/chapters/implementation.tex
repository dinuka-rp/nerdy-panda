...
% Used as an alternative to using a framework since the vast array of capabilities and other integratable frameworks and libraries. 
...
The code segment in Fig \ref{fig:code-preprocess-trends} preprcoesses trends that are fetched from the live Twitter \gls{api}.

% tweet volume \& median
The code segment in Fig \ref{fig:code-tweet-volume-calculation} of \textit{\nameref{appendix:implementation}} assigns a tweet volume for trends with no volume \& calculates the median Tweet volume which used to calculate the impact score of each trend.
...
% add testing, eval code to appendix and ref here under a new subsection
All code snippets that were created for Testing \& Evaluation purposes have been mentioned under \textit{\nameref{appendix:implementation}}
...
% \section{Research Product Documentation}
